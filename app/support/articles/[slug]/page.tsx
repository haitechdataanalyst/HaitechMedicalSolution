        ...
        <article className="mx-auto max-w-4xl py-2 sm:py-4">
            ...
            <header className="mb-8">
                ...
                <div className="mb-4 flex items-center gap-2">
                    <Tag className="text-primary-600 h-4 w-4" aria-hidden="true" />
                    <span className="text-primary-600 text-sm font-medium tracking-wide uppercase">{article.category}</span>
                </div>

                {/* Title */}
                <h2 className="mb-4 text-3xl leading-tight font-bold text-gray-900 sm:text-4xl">{article.title}</h2>
            ...
            {/* Article Navigation */}
            <nav aria-label="Article navigation" className="mt-12 border-t border-gray-200 pt-8">
                <div className="flex items-center justify-between">
                    {prevArticle ? (
                        <Link href={`/support/articles/${prevArticle.slug}`} className="hover:text-primary-600 group flex items-center gap-3 text-gray-600 transition-colors">
                            <ChevronLeft className="h-5 w-5 transition-transform group-hover:-translate-x-1" aria-hidden="true" />
                            <div className="text-left">
                                <span className="text-xs font-medium tracking-wide text-gray-500 uppercase">Previous Article</span>
                                <span className="line-clamp-2 block text-sm font-medium">{prevArticle.title}</span>
                            </div>
                        </Link>
                    ) : (
                        <div />
                    ...
                    {nextArticle ? (
                        <Link href={`/support/articles/${nextArticle.slug}`} className="hover:text-primary-600 group flex items-center gap-3 text-right text-gray-600 transition-colors">
                            <div>
                                <span className="text-xs font-medium tracking-wide text-gray-500 uppercase">Next Article</span>
                                <span className="line-clamp-2 block text-sm font-medium">{nextArticle.title}</span>
                            </div>
            ...
            <aside className="mt-12 border-t border-gray-200 pt-8 text-center">

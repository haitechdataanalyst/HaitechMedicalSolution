...
export const isStraussGritProduct = (product: Product): boolean =>
    Boolean(product.hasVariants && product.variantType === "grit" && product.variants?.length && STRAUSS_CATEGORY_IDS.includes(product.category));
...
export const isLegacyVariantProduct = (product: Product): boolean =>
    Boolean(product.hasVariants && product.variants && product.variants.length > 0 && product.variantType !== "frame-color" && !isStraussGritProduct(product));

/**
 * Check if a product has frame variants
 */
export const hasFrameVariants = (product: Product, framesCount: number): boolean => product.variantType === "frame-color" && Boolean(product.frameVariants) && framesCount > 0;

 ...
 */
export default function StraussProductBlock({ product, selectedVariant, onVariantSelect, showSpecifications = true }: StraussProductBlockProps) {
    if (!product.variants) {
        return null;
    }

    // Extract specifications from product content blocks
    const specsBlock = product.contentBlocks?.find((b): b is ContentBlock & { type: "specifications" } => b.type === "specifications");
    ...
    return (
        <div className="space-y-6">
            <StraussGritSelector variants={product.variants} selectedVariant={selectedVariant} onVariantSelect={onVariantSelect} />
        </div>
    );
}

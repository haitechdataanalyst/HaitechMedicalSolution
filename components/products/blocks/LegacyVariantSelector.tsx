...
export default function LegacyVariantSelector({
  variants,
  variantType,
  selectedVariant,
  onVariantSelect,
}: LegacyVariantSelectorProps) {
  const isColorType = variantType === "color";

  return (
    <div>
      <label className="mb-3 block text-sm font-medium text-neutral-700">
        {isColorType ? "Select Color" : "Select Variant"}
      </label>
        ...
        {variants.map((variant) => {
          ...
          return (
            ...
            >
              ...
              {isColorType && variant.name && (
                <span
                  className={`text-xs ${
                    isSelected ? "font-medium text-primary-700" : "text-muted"
                  }`}
                >
                  {variant.name}
                </span>
      ...
      {isColorType && selectedVariant && (
        <p className="text-muted mt-2 text-sm">
          Selected: {selectedVariant.name ?? selectedVariant.id}
        </p>
      )}
    </div>
  );

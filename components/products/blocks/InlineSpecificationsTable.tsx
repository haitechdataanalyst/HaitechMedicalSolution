 ...
 */
export default function InlineSpecificationsTable({ title, rows }: InlineSpecificationsTableProps) {
    if (rows.length === 0) {
        return null;
    }
    ...
    return (
        <div>
            {title && <h3 className="mb-3 text-sm font-semibold text-neutral-700">{title}</h3>}

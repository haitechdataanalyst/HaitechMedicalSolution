...
export default function MatchHeadlightsSection({ config, headlightCategories }: MatchHeadlightsProps) {
  ...
  const filteredCategories = config.categories
    ? headlightCategories.filter((c) => config.categories!.includes(c.id))
    : headlightCategories;
              ...
              <button
                ...
                className={cn(
                  ...
                  activeCategoryId === category.id
                    ...
                    : "border border-neutral-300 bg-white text-neutral-600 hover:border-primary-300 hover:text-primary-600 cursor-pointer"
                )}
              >
                {category.name}
              </button>
          ...
          <div className="flex gap-4 overflow-x-auto pb-2">
            {products.map((product) => (
              <div key={product.id} className="flex min-w-30 shrink-0 flex-col items-center gap-2">
                ...
                <Link
                  href={`/our-headlights`}
                  className="inline-flex items-center gap-0.5 text-xs text-primary-600 hover:text-primary-700 transition-colors"
                >
